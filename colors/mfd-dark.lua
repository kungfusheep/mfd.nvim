-- MFD Dark: Military Flight Display theme for Neovim (dark variant)
-- Monotone olive/sage with text decoration for syntax - dark background

vim.cmd('highlight clear')
vim.g.colors_name = 'mfd-dark'

local c = {
  bg       = '#1E2D1E',  -- dark forest green
  fg       = '#8A9B70',  -- light olive
  dim      = '#3A4A3A',  -- subtle olive (comments)
...
hi('DiffAdd',      { bg = '#2A3D2A' })
hi('DiffChange',   { bg = '#2D3D2D' })
hi('DiffDelete',   { fg = c.dim, bg = '#3D2D2D' })
...
hi('GitSignsAddInline',     { bg = '#2A3D2A' })
hi('GitSignsDeleteInline',  { bg = '#3D2D2D' })
